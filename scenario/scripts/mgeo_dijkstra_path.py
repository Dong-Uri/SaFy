...
class dijkstra_path_pub :
    ...
    def init_callback(self,msg):
        ...
        for node_id, node in self.nodes.items():
            dist = ((msg.pose.pose.position.x - node.point[0]) ** 2 + (msg.pose.pose.position.y - node.point[1]) ** 2) ** 0.5
            if dist < min_dist:
                min_dist = dist
                node_idx = node_id
    ...
    def goal_callback(self,msg):
        ...
        min_dist = float('inf')
        for node_id, node in self.nodes.items():
            dist = ((msg.pose.position.x - node.point[0]) ** 2 + (msg.pose.position.y - node.point[1]) ** 2) ** 0.5
            if dist < min_dist:
                min_dist = dist
                node_idx = node_id
...
class Dijkstra:
    ...
    def find_shortest_link_leading_to_node(self, from_node,to_node):
        ...
        return from_node.find_shortest_link_leading_to_node(to_node)
